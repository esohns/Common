 ...
 : id_ (-1)
{
  ...
  if (unlikely (!load (vertexShaderFilename_in,
                       fragmentShaderFilename_in)))
  {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to Common_GL_Shader::load(\"%s\",\"%s\"), returning\n"),
...
bool
Common_GL_Shader::load (const std::string& vertexShaderFilename_in,
                        const std::string& fragmentShaderFilename_in)
{
  COMMON_TRACE (ACE_TEXT ("Common_GL_Shader::load"));
  ...
  if (unlikely (!Common_GL_Tools::loadAndCompileShader (vertexShaderFilename_in,
                                                        GL_VERTEX_SHADER,
                                                        vertex_shader_id_i)))
  {
    ACE_DEBUG ((LM_ERROR,
                ACE_TEXT ("failed to Common_GL_Tools::loadAndCompileShader(\"%s\"), aborting\n"),
                ACE_TEXT (vertexShaderFilename_in.c_str ())));
    return false;
  } // end IF
  if (unlikely (!Common_GL_Tools::loadAndCompileShader (fragmentShaderFilename_in,
                                                        GL_FRAGMENT_SHADER,
