...
// initialize statics
bool Common_SDL_Tools::videoIsPreInitialized = false;

bool
Common_SDL_Tools::preInitializeVideo (const struct Common_SDL_VideoConfiguration& configuration_in,
                                      const std::string& caption_in)
{
  COMMON_TRACE (ACE_TEXT ("Common_SDL_Tools::preInitializeVideo"));

  // sanity check
  if (Common_SDL_Tools::videoIsPreInitialized)
  ...
  {
    char* environment =
      ACE_OS::getenv (ACE_TEXT_ALWAYS_CHAR (COMMON_SDL_VIDEO_DRIVER_ENV_VAR));
    if (environment)
      video_driver = environment;
  } // end IF
  // sanity check
  if (video_driver.empty ())
    video_driver = ACE_TEXT_ALWAYS_CHAR (COMMON_SDL_VIDEO_DEFAULT_DRIVER_NAME); // fallback
  ACE_DEBUG ((LM_DEBUG,
  ...
  Common_SDL_Tools::videoIsPreInitialized = true;
...
#if defined (SDL_USE)
bool
Common_SDL_Tools::initializeVideo (const struct Common_SDL_VideoConfiguration& configuration_in,
                                   const std::string& caption_in,
                                   SDL_Surface*& windowSurface_out,
                                   bool initWindow_in)
{
  COMMON_TRACE (ACE_TEXT ("Common_SDL_Tools::initializeVideo"));
  ...
  if (!myVideoPreInitialized)
    ...
    {
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("failed to Common_SDL_Tools::preInitVideo(), aborting\n")));
      return false;
    } // end IF
  ...
  if (initWindow_in)
  {
    windowSurface_out = Common_SDL_Tools::initializeScreen (configuration_in);
    if (!windowSurface_out)
    {
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("failed to Common_SDL_Tools::initScreen(%d,%d,%d), aborting\n"),
...
SDL_Surface*
Common_SDL_Tools::initializeScreen (const struct Common_SDL_VideoConfiguration& configuration_in)
{
  COMMON_TRACE (ACE_TEXT ("Common_SDL_Tools::initScreen"));
                                   ...
                                   bool initializeWindow_in)
{
  ...
  // sanity check
  if (!Common_SDL_Tools::videoIsPreInitialized)
    if (!Common_SDL_Tools::preInitializeVideo (configuration_in,
                                               caption_in))
    {
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("failed to Common_SDL_Tools::preInitializeVideo(), aborting\n")));
      return false;
    } // end IF
  ACE_ASSERT (Common_SDL_Tools::videoIsPreInitialized);

  // step3: initialize screen
  if (initializeWindow_in)
  {
    if (!Common_SDL_Tools::initializeScreen (configuration_in,
                                             caption_in,
                                             windowHandle_out,
                                             rendererHandle_out,
                                             GLContextHandle_out))
    {
      ACE_DEBUG ((LM_ERROR,
                  ACE_TEXT ("failed to Common_SDL_Tools::initializeScreen(%d,%d,%d), aborting\n"),
                  configuration_in.screen_width,
                  configuration_in.screen_height,
                  configuration_in.screen_colordepth));
      return false;
...
{
  COMMON_TRACE (ACE_TEXT ("Common_SDL_Tools::initializeScreen"));
...
#if defined (SDL_USE)
std::string
Common_SDL_Tools::keyToString (const SDL_keysym& key_in)
{
  COMMON_TRACE (ACE_TEXT ("Common_SDL_Tools::keyToString"));
...
Common_SDL_Tools::keyToString (const SDL_Keysym& key_in)
{
  ...
  char buffer[COMMON_SDL_KEYSYM_BUFFER_SIZE];
  result += ACE_TEXT_ALWAYS_CHAR ("scancode: ");
  ACE_OS::sprintf (buffer,
                   ACE_TEXT_ALWAYS_CHAR ("0x%02X"),
                   key_in.scancode);
...
struct SDL_Color
Common_SDL_Tools::colorToSDLColor (Uint32 SDLColor_in,
                                   const struct SDL_PixelFormat& pixelFormat_in)
{
  COMMON_TRACE (ACE_TEXT ("Common_SDL_Tools::colorToSDLColor"));
...
SDL_Rect
Common_SDL_Tools::boundingBox (const struct SDL_Rect& rect1_in,
                               const struct SDL_Rect& rect2_in)
{
  COMMON_TRACE (ACE_TEXT ("Common_SDL_Tools::boundingBox"));
...
SDL_Rect
Common_SDL_Tools::intersect (const struct SDL_Rect& rect1_in,
                             const struct SDL_Rect& rect2_in)
{
  COMMON_TRACE (ACE_TEXT ("Common_SDL_Tools::intersect"));
...
bool
Common_SDL_Tools::equal (const struct SDL_Rect& rect1_in,
                         const struct SDL_Rect& rect2_in)
{
  COMMON_TRACE (ACE_TEXT ("Common_SDL_Tools::equal"));
...
SDL_Rect
Common_SDL_Tools::difference (const struct SDL_Rect& rect1_in,
                              const struct SDL_Rect& rect2_in)
{
  COMMON_TRACE (ACE_TEXT ("Common_SDL_Tools::difference"));
  ...
  // sanity check(s)
  struct SDL_Rect temp = Common_SDL_Tools::intersect (rect1_in, rect2_in);
  if ((temp.x == 0) &&
      (temp.y == 0) &&
      (temp.w == 0) &&
      (temp.h == 0))
    return temp; // no overlap
  else if (Common_SDL_Tools::equal (temp, rect1_in))
